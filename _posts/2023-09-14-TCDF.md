...
네트워크의 hidden layer 수가 증가하면 일반적으로 training error가 증가한다고 합니다. 이러한 정확도 저하 문제는 과적합으로 인한 것이 아니라 backpropagation이 최적의 가중치를 찾지 못하기 때문에 발생합니다. 이 문제를 해결하기 위해 residual connection을 사용합니다. convolution layer를 통과한 입력 $x$ 를 $F(x)$ 라고 한다면, residual connection과 활성함수 PReLU를 통과한 output $\sigma$는 다음과 같습니다.  

$\begin{align} \sigma=PReLU(x+F(x)) \end{align}$
