...
$\begin{align} h=\begin{cases} \sigma(a), & if \ a \geq \tau_j
\\ 0, & if \ a < \tau_j \end{cases} \end{align}$ 

\[
\begin{aligned}
h = 
\begin{cases} 
\sigma(a), & \text{if } a \geq \tau_j \\
0, & \text{if } a < \tau_j 
\end{cases}
\end{aligned}
\]
...
본 논문에서는 potential cause의 집합인 $P_j$를 정의합니다. 만약 시계열 $X_i$가 target 시계열 $X_j$의 잠재적 원인이라면 