...
본 논문에서의 **Temporal causal discovery**는 길이가 $T$이고, 변수의 개수가 $N$인 시계열 데이터 $\mathbf{X}=\{\mathbf{X}_1,\mathbf{X}_2,...,\mathbf{X}_N\} \in \mathbb{R}^{N \times T}$가 주어졌을 때, X의 모든 N개의 시계열과 원인과 결과 사이의 시간 지연 사이의 인과관계를 발견하고 시간적 인과관계 그래프로 모형화하는 것으로 정의합니다.  
인과그래프 $g=(V,E)$ 에서 노드 $v_i \in V$는 $i$번째 시계열 데이터 $\mathbf{X_i}$를 의미하고, 엣지 $e_{i,j}$는 노드 $v_i$와 노드 $v_j$의 관계를 나타냅니다. 즉, $\mathbf{X_i}$가 $\mathbf{X_j}$의 원인임을 나타냅니다.  
