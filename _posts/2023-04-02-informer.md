...
1. LSTF Problem 정의
    ...
    * Input : $X_t=\{x_1^t,...,x_{L_x}^t|x_i^t \in \mathbb{R}^{d_x}\}$  , Ouput : $Y_t=\{y_1^t,...,y_{L_y}^t|y_i^t \in \mathbb{R}^{d_y}\}$
...
2. 어떻게 유의미한 dot-product pair를 구분할 것인가?
    * 유의미한 query($i$)-key(모든 $j$) pairs의 분포는 uniform 분포와 상이합니다. 만약 $p(k_j|q_i)$의 분포가 uniform분포인 $q(k_j|q_i)=1/L_K$에 근사하다면, self-attention score로 value V 가중합에 trivial한 영향을 주게 되고 상대적으로 불필요한 query로 작용하게 됩니다.
